...
struct CartScreen: View {
    ...
    @State private var celebrationOffer: RetailerAppliedOffer? = nil
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showSchemeSheet) {
            SchemePickerSheet(
                ...
                onSchemeApplied: { applied in
                    loadAvailableOffers()
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.35) {
                        celebrationOffer = applied
...
struct ConfettiView: View {
    ...
    private let colors: [Color] = [
        ...
        Color(hex: "#167444"), // Spice Primary
        Color(hex: "#F59E0B"), // Amber Gold
        Color(hex: "#3B82F6"), // Sky Blue
        ...
        TimelineView(.animation) { _ in
            Canvas { context, size in
                for piece in pieces {
                    ...
                    let path: Path = piece.isCircle ? Path(ellipseIn: rect) : Path(roundedRect: rect, cornerRadius: 2)
    ...
    private func spawnConfetti() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 0.016, repeats: true) { _ in
            for i in pieces.indices {
                ...
                pieces[i].vy += 0.35 // gravity
                pieces[i].vx *= 0.98 // air drag
                pieces[i].rotation += pieces[i].vRot
            }
        }
    }
}

// MARK: - Scheme Celebration Modal View

struct SchemeCelebrationModalView: View {
    let offer: RetailerAppliedOffer
    let onDismiss: () -> Void
    ...
    var schemeCode: String {
        let raw = offer.schemeTitle ?? offer.title ?? "OFFER"
        return raw.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
    }

    var savingsText: String {
        if let amt = offer.discountAmount, amt > 0 {
            return String(format: "You saved ₹%.2f on this order", amt)
        }
        if let gift = offer.giftDescription, !gift.isEmpty {
            return "You unlocked a Free Gift: \(gift)"
        }
        if let text = offer.discountText, !text.isEmpty {
            return text
        }
        return "Special scheme discount applied on this order"
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                // Success Checkmark Circle with Radiant Pulsing Rings
                ZStack {
                    Circle()
                        .fill(Color(hex: "#167444").opacity(0.18))
                        .frame(width: 84, height: 84)
                        .scaleEffect(ringPulse ? 1.2 : 0.85)
                        .opacity(ringPulse ? 0.0 : 0.8)
                        .animation(Animation.easeOut(duration: 1.4).repeatForever(autoreverses: false), value: ringPulse)

                    Circle()
                        .fill(Color(hex: "#167444"))
                        .frame(width: 64, height: 64)
                        .shadow(color: Color(hex: "#167444").opacity(0.35), radius: 10, x: 0, y: 5)
                ...
                // Scheme Code / Title (Tracked Caps)
                Text(schemeCode)
                ...
                // "Applied successfully" Headline
                Text("Applied successfully")
                    .font(.appFont(size: 19, weight: .heavy))
                    .foregroundColor(Color(hex: "#167444"))
                    .padding(.top, 6)

                // "You saved ₹... on this order" Subtitle
                Text(savingsText)
            ...
            .background(Color.white)
            .cornerRadius(24)
            .shadow(color: Color.black.opacity(0.22), radius: 30, x: 0, y: 12)
